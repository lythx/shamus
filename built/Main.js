...
import { renderDebug, renderIntro, renderUi, renderUnits, removeIntro, removeStart, displayPause, removePause, displayWin } from "./Renderer.js";
...
import { oppositeDirection4 } from "./utils/Directions.js";
...
const spawnEnemies = (units, spawnAreas) => {
    ...
    for (const [name, count] of Object.entries(units)) {
        const UnitClass = unitClasses[name];
...
const onRoomChange = (roomNum, sideOrPos) => {
    AudioPlayer.stop();
    AudioPlayer.resume();
...
const handleIntroKeydown = (key) => {
    if (config.controls.keyboard.changeDifficulty === key ||
        config.controls.gamepad.changeDifficulty.toString() === key) {
        difficulty = config.difficulties[(config.difficulties.indexOf(difficulty) + 1) % config.difficulties.length];
...
const restartGame = () => {
    ...
    Timer.resume();
    removeIntro();
    onRoomChange(config.room.start, config.room.startSide);
...
const shadowPositions = [[0, 0], [1400, 0], [0, 800], [1400, 800]];
const spawnShadow = () => {
    shadowSpawned = true;
...
const gameLoop = () => {
    ...
    if (debug) {
        ...
        for (let i = 0; i < Enemy.enemies.length; i++) {
            ...
            for (let j = 0; j < e.debug.length; j++) {
                objects[index++] = e.debug[j];
            }
        }
        for (let i = 0; i < room.edges.length; i++) {

        ...
        if (itemObj === undefined) {
            ...
            if (itemObj.type === 'key') {
                if (keyType !== undefined) {
                    item = { obj: new GameKey(keyType), pos };
                }
                else {
                    itemObj.type = 'random';
                }
            ...
            if (itemObj.type === 'mystery') {
                ...
                item = {
                    obj: new KeyHole(itemObj.color, edgeToDelete), pos
                };
            }
        }
...
const loadRoom = (roomNumber, usedEntrance) => {
    const newRoom = rooms[roomNumber];
    if (newRoom === undefined) {
        throw new Error(`Room ${roomNumber} doesn't exist`);
    }
...
export const roomManager = {
    initialize,
    loadRoom,
    circleCollision: (c, checkInsides) => room.circleCollision(c, checkInsides),
    vectorCollision: (v) => room.vectorCollision(v),
    checkIfOnEntrance: (playerHitbox) => room.checkIfOnEntrance(playerHitbox)
};
